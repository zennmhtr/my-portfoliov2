...
export default function ContactSection() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const serviceId =
        process.env.NEXT_PUBLIC_EMAILJS_SERVICE_ID || "service_jgil3xk";
      const templateId =
        process.env.NEXT_PUBLIC_EMAILJS_TEMPLATE_ID || "template_o6dfowq";
      const publicKey =
        process.env.NEXT_PUBLIC_EMAILJS_PUBLIC_KEY || "syzitMr_PIyjy1Me6";
      ...
      const result = await emailjs.send(
        ...
        templateId,
        {
          name_initial: formData.name.split(' ').map((n: string) => n[0]).join('').toUpperCase().slice(0, 2),
